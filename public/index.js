...
async function guides() {
  ...
  if (currentGuideView) {
    document.getElementById(
      `guide${guides.findIndex((g) => g.name == currentGuideView.guide.name)}`
    ).checked = true;
  }
}
...
async function newGuide() {
  openModal(
    templates["create-guide-template"]({
      champs: await (await fetch("/riot/open-champs")).json(),
    })
  );
  canCreateGuide();
}
...
function newPage() {
  ...
  storeGuideValues() &&
    renderGuide(currentGuideView.guide.name, "contents", pageLen);
}

function removePage() {
  delete currentGuideView.guide.contents[
    Object.keys(currentGuideView.guide.contents)[currentGuideView.pageTab]
  ];
  if (
    currentGuideView.pageTab >=
    Object.keys(currentGuideView.guide.contents).length
  )
    currentGuideView.pageTab--;
  renderGuide(
    currentGuideView.guide.name,
    "contents",
    currentGuideView.pageTab
  );
}
...
function renamePage() {
  ...
  const oldPage = Object.keys(currentGuideView.guide.contents)[
    currentGuideView.pageTab
  ];
  ...
  delete Object.assign(currentGuideView.guide.contents, {
    [newPage]: currentGuideView.guide.contents[oldPage],
  })[oldPage];
  let currentOrder = Object.keys(currentGuideView.guide.contents);
  currentOrder = [
    ...currentOrder.slice(0, currentGuideView.pageTab),
    newPage,
    ...currentOrder.slice(currentGuideView.pageTab, currentOrder.length - 1),
  ];
  currentGuideView.guide.contents = _preferredOrder(
    currentGuideView.guide.contents,
    currentOrder
  );
...
function movePage(d) {
  ...
  const next =
    (currentGuideView.pageTab + d + currentOrder.length) % currentOrder.length;
  [currentOrder[currentTab], currentOrder[next]] = [
    currentOrder[next],
    currentOrder[currentTab],
  ];
  currentGuideView.guide.contents = _preferredOrder(
    currentGuideView.guide.contents,
    currentOrder
...
async function renderImage() {
  ...
  imgEl.src = await urlContentToDataUri(
    `/guide/image/${currentGuideView.guide.name}`
  );
  imgEl.width = 482;
}
...
async function renderGuide(champion, tab = "properties", pageTab = 0) {
  const guide =
    currentGuideView?.guide?.name == champion
      ? currentGuideView.guide
      : await (await fetch(`/guide/${champion}`)).json();
  currentGuideView = { guide, isPropertiesTab: tab == "properties", pageTab };
  guideContentDiv.innerHTML =
    templates["guide-content-template"](currentGuideView);
  ...
  if (currentGuideView.isPropertiesTab) {
    ...
    document
      .getElementById(`diff-${guide.image.difficulty}`)
      .setAttribute("selected", true);
    insertRunes();
    guide.image.runes.forEach((v) =>
    ...
    document.querySelectorAll(".rune").forEach((runeBtn, i) => {
      runeBtn.onclick = () => {
        ...
        const rowDiv = [
          ...document.getElementById(`rune-select`).children[row].children,
        ];
        let active = rowDiv.find((btn) => btn.classList.contains("active"));
        if (active) {
          active.classList.remove("active");
        } else {
          const other = [...document.getElementsByClassName("rune")].findIndex(
            (e) => e.classList.contains("active")
          );
          const otherRow = Math.floor(other / 3);
          document
            .querySelectorAll(".rune.active")
            [swapperoo].classList.remove("active");
    ...
    document.getElementById("page-text").value = Object.values(guide.contents)[
      pageTab
    ];
    Object.keys(guide.contents).length == 4 &&
      document.getElementById("new-page").setAttribute("disabled", true);
    Object.keys(guide.contents).length == 1 &&
      document.getElementById("delete-page").setAttribute("disabled", true);
  }
}
...
function storeGuideValues() {
  ...
  if (currentGuideView.isPropertiesTab) {
    ...
    currentGuideView.guide.image.starter =
      starterOptions[starterOptions.selectedIndex].id;
    currentGuideView.guide.image.difficulty =
      document.getElementById("difficulty").selectedIndex + 1;
    currentGuideView.guide.image.smallText =
      document.getElementById("optional_text").value;
    currentGuideView.guide.image.primaryRune =
      document.getElementById("primary_rune").value;
    currentGuideView.guide.image.sums = document.getElementById("sums").value;
    currentGuideView.guide.image.runes = [
      ...document.querySelectorAll(".rune.active"),
    ].map((e) => e.id);
  } else {
    currentGuideView.guide.contents[
      Object.keys(currentGuideView.guide.contents)[currentGuideView.pageTab]
...
function renderVisibility() {
  document.getElementById("visibility").textContent = currentGuideView.guide
    .public
    ? "Public"
    : "Private";
}
...
async function canCreateGuide() {
  ...
  const list = [...document.getElementById("champs").children].map(
    (c) => c.value
  );

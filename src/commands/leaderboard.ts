...
async function renderLeaderboard(client: Client<true>) {
    const leaderboard = 
        ...
        .slice(0, LIMIT)
        .map((pair, i) => [withPlacePrefix(i + 1, pair[0]), withRankEmoji(pair[1])])
    const lines = (v: string[]) => v.reduce((a, b) => `${a}\n${b}`, "").slice(1)
    return {
        embeds: [
            new EmbedBuilder({
                color: Colors.Aqua,
                title: `SoloQ Leaderboards (Top ${LIMIT})`, fields: [
                    { name: "Players", value: lines(Object.values(leaderboard).map((pair) => pair[0])), inline: true },
                    { name: "Rank", value: lines(Object.values(leaderboard).map((pair) => pair[1])), inline: true }
                ],
...
export default {
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        try {
            await interaction.reply({ content: "Sending leaderboard...", ephemeral: true })
            let res = await interaction.channel!.send(await renderLeaderboard(interaction.client))
            await setLeaderboardMessageLocation({ channelId: interaction.channelId, messageId: res.id })
    ...
    button: async (interaction: ButtonInteraction, metadata: ButtonMetadata) => {
        try {
            ...
            const favoriteColorInput = new TextInputBuilder()
                ...
                .setValue(leaderboardEntry?.id ? (await getSummonerNickname(regionFromStr(leaderboardEntry.region)!, leaderboardEntry.id) ?? "") : "")
                .setRequired(false)
            ...
            const secondActionRow = new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(serverInput);
    ...
    modalSubmit: async (interaction: ModalSubmitInteraction) => {
        try {
            ...
            const summonerName = interaction.fields.getTextInputValue('summonerName').trim(),
                server = interaction.fields.getTextInputValue('server').trim().toLowerCase()

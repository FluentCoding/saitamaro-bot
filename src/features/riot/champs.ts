...
export async function getNeutralChampionName(
  champion: Champion,
): Promise<NeutralChampion> {
  const result = await fetchCachedJson(
    `${CDN_BASE_URL}/${patch}/data/en_US/champion.json`,
  );
  return Object.entries(result.data).find(
    (kv: any) => kv[1].name == champion,
  )?.[0] as NeutralChampion;
}
...
export async function getChampion(champion: NeutralChampion) {
  return await fetchCachedJson(
    `${CDN_BASE_URL}/${patch}/data/en_US/champion/${champion}.json`,
  );
}
...
export async function allSkins(
  neutralChampion: NeutralChampion,
): Promise<number[]> {
  return (await getChampion(neutralChampion)).data[neutralChampion].skins.map(
    (skin: any) => skin.num,
  );
}

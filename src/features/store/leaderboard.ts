...
export type LeaderboardMessagesLocation = {
  channelId: string;
  messageIds: string[];
};
...
export function getLeaderboardMessageLocation() {
  return db.getObjectDefault<LeaderboardMessagesLocation | undefined>(
    "/messageIds",
    undefined
  );
}
...
export function setLeaderboardMessageLocation(
  data: LeaderboardMessagesLocation
) {
  return db.push("/messageIds", data);
}
